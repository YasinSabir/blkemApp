...
class ResetPasswordCubit extends Cubit<CommonUIState> {
  ...
  ResetPasswordCubit(this.resetPasswordUseCase) : super(CommonUIState.initial()){
  ...
  resetPassword()async{
    if(emailValidator.isEmpty){
      ...
      var response=await resetPasswordUseCase(emailValidator.text);
      emit(response.fold((l) => CommonUIState.error(l.errorMessage), (r) => CommonUIState.success("Password link sent successfully")));
    }
  }
}
